...
const MediaSection = ({ printMedia, updates, isAdmin }) => {
  ...
  return (
    <div className="w-[55%] h-full pt-3 px-4 max-[600px]:px-2 max-[600px]:w-full flex items-center justify-center">
      <div className="border border-gray-300 flex flex-col items-center h-[96%] w-[98%] rounded-md relative p-2">
        ...
        {isAdmin && <button onClick={() => navigate("/admin/edituploads/Print Media")} className='absolute bottom-5 left-[4%] z-20 border border-white bg-blue-800 py-0.5 px-1 rounded-md cursor-pointer text-white  text-md hover:bg-blue-600'>Edit print media</button>}

        {isAdmin && <button onClick={() => navigate("/admin/edituploads/updates @")} className='absolute bottom-5 right-[4%] z-20 border border-white bg-blue-800 py-0.5 px-1 rounded-md cursor-pointer text-white  text-md hover:bg-blue-600'>Edit updates @</button>}
