...
// src/pages/admin/EditNotificationPage.jsx
import React, { useState, useEffect } from 'react';
import { createNotification, updateNotification, deleteNotification } from '../../api/axios';


export default function EditNotificationPage({ notifications }) {
    const [editing, setEditing] = useState(null);
    const [form, setForm] = useState({ title: '', link: '' });
    const [items, setItems] = useState(notifications || []); // local state copy
    ...
    const submit = async (e) => {
        ...
        try {
            if (editing) {
                const updated = await updateNotification(editing._id, form);
    ...
    return (
        <div className="p-6">
            ...
            <form onSubmit={submit} className="mb-6 max-w-xl bg-white p-4 rounded shadow">
                <h3 className="mb-2 font-semibold">{editing ? 'Edit Notification' : 'Create Notification'}</h3>
